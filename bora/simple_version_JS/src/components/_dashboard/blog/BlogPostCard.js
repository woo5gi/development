...
export default function BlogPostCard({ post, index }) {
  ...
  useEffect(() => {
    axios
      .get('/bora/blog/posts')
      .then((result) => setMainContents(result.data))
      .catch((err) => console.error(err));
  }, []);
  console.log({ mainContents });

  const imgList = mainContents.map((mainContent, index) => (
    <div>
      <CardMediaStyle
        sx={{
  ...
  /> */}
        ...
        <CoverImgStyle alt={mainContent.title} src={`http://115.85.180.18:9000/static/assets/${mainContent.key}`} />
      </CardMediaStyle>
  ...
  return (
    <Grid item xs={12} sm={latestPostLarge ? 12 : 6} md={latestPostLarge ? 6 : 3}>
      <Card sx={{ position: 'relative' }}>{imgList}</Card>
    </Grid>
  );
}
